...
| `/check` | Is this an RTI matter? Verdict + reasoning, or an honest "no, go here instead" | from `/` | "Am I in the right place?" | domain taxonomy + interview answers | Deterministic, no network | — | — | **Not built** |
| `/draft` | The drafted request: shown in full, explained, editable | from `/check` | "Ask it properly" | domain templates + character rules | Deterministic, no network | — | — | **Not built** |
| `/authority` | Which office — one recommendation, reasoning, ranked alternatives, search | from `/draft` | "Send it to the right place" | public-authority dataset | Real names, deterministic ranked search | — | — | **Not built** |
